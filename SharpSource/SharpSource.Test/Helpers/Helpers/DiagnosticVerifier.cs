...
public abstract class DiagnosticVerifier
{
    ...
    {
        if (CodeFixProvider != default)
        {
            _codeFixVerifier.VerifyFix(CodeFixProvider, DiagnosticAnalyzer, oldSource, newSource, codeFixIndex, allowNewCompilerDiagnostics);
        }
    } 

    protected void VerifyFix(string oldSource, string newSource, int? codeFixIndex = null, params string[] allowedNewCompilerDiagnosticsId)
    {
        if (CodeFixProvider != default)
        {
            _codeFixVerifier.VerifyFix(CodeFixProvider, DiagnosticAnalyzer, oldSource, newSource, codeFixIndex, allowedNewCompilerDiagnosticsId);
        }         
    }
}