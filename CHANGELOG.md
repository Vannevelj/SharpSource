...
- `PointlessCollectionToString`, `ThrowNull`, `ExplicitEnumValues`, `RethrowExceptionWithoutLosingStracktrace`, `SwitchDoesNotHandleAllEnumOptions`, `ParameterAssignedInConstructorTests`, `EqualsAndGetHashcodeNotImplemented`, `TestMethodWithoutPublicModifier`, `StructWithoutElementaryMethodsOverridden` and `UnboundedStackalloc` tests use `VerifyCS`
